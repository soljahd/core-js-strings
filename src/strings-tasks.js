...
function countVowels(str) {
  const vowelsMatch = str.match(/[aeiouyAEIOUY]/g) || [];
  return vowelsMatch.length;
}
...
function isPalindrome(str) {
  const cleenStr = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  return cleenStr === cleenStr.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const index = englishAlphabet.indexOf(str[i]);
